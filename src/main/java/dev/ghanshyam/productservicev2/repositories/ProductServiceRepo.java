...
public interface ProductServiceRepo extends JpaRepository<Product, UUID> {
    ...
    @Query("select p from Product p LEFT JOIN Category c ON p.category=c where c.category=:cat_name")
...
//    public List<Product> getProductByCategoryIn(List<Category>categories);
